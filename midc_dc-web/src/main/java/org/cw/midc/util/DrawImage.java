...
public class DrawImage {
	...
	public DrawImage() {
		...
		// _file.getName();
		// this.destFile = "E:/temp/dage2.jpg";// this.srcFile.substring(0,
	 ...
	 * @throws IOException
	 */
	public void resizeAndBak(String bakAbsoluteName, String absoluteName,
			int newWidth, int newHeight) throws IOException {
		File _file = new File(absoluteName); // 读入文件
		_file.getName();
		this.destFile = absoluteName + "_s";
		img = javax.imageio.ImageIO.read(_file); // 构造Image对象
		width = img.getWidth(null); // 得到源图宽
		height = img.getHeight(null); // 得到源图长
		resizeFix(newWidth, newHeight);
		new File(bakAbsoluteName).delete();
		new File(absoluteName).renameTo(new File(bakAbsoluteName));
		new File(this.destFile).renameTo(new File(absoluteName));
	}
}